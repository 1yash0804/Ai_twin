...
export default function LoginPage() {
  ...
  const handleRequestOtp = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      await requestLoginOtp(username, password);
      setStep(2);
    } catch (err) {
  ...
  // Step 2 — verify OTP, get token, redirect
  const handleVerifyOtp = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setError(null);
    setLoading(true);
    try {
      const session = await verifyLoginOtp(username, otp);
        ...
        <div className="w-full max-w-md bg-white border border-slate-200 rounded-[2.5rem] p-12 shadow-2xl shadow-slate-200/50">
          ...
          <div className="text-center mb-10 space-y-4">
            ...
            <p className="text-slate-500 font-medium">
              {step === 1
                ? "Sign in to your AI Twin account."
